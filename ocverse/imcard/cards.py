...
def help_card(cfg: dict, sub_prefix: str = "/分身") -> list[Image.Image]:
    ...
    rows = [
        sec("🎪 开始之前", [
            f"管理员先把世界铺好:{sub_prefix} 初始化世界 [世界观描述…](不填则 AI 自由发挥)",
            f"创建分身:{sub_prefix} 创建 <名字> [设定描述…] — 一句话自由描述,AI 自动整理人设与初始属性;一人一个",
            f"头像:{sub_prefix} 设置头像 (随指令发一张图,或回复一张图)",
        ]),
        sec("🗣 最推荐:直接用大白话", [
            "不用背指令,想做什么就说出来(普通聊天里即可,或加前缀更稳):",
            "· 「帮我看看我的角色」/「现在的世界是什么样」",
            "· 「我想去茶馆喝杯茶」/「和绫波婆婆聊聊天」",
            "· 「练练剑」/「去打个怪」/「接今天的任务」/「交第1个任务」",
            "· 「买第2套房然后回家」/「穿越到锈海城」",
            f"· 拿不准时:{sub_prefix} 说 <你想做什么>,AI 会一步步帮你做完",
        ]),
        sec("📜 每天的生活(事件与卡片)", [
        ...
        sec("🧩 其他", [
            ...
            f"🎭 {sub_prefix} 定义角色 <名字> <描述> — 造一位持久生活角色;{sub_prefix} 找 <名字> 互动(玩家/生活角色通用)",
