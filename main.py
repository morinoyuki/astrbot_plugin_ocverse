...
class OcversePlugin(Star):
    ...
    async def cmd_create(self, event: AstrMessageEvent):
        ...
        if "|" in rest:
            ...
            backstory = parts[3][:1200] if len(parts) > 3 else ""
        else:
            ...
            if desc:
                ...
                if r.ok:
                    ...
                    backstory = desc[:1200]  # AI 不可用:描述原文入背景,不丢信息
        async with self._glock(gid):
    ...
    async def cmd_edit(self, event: AstrMessageEvent):
        ...
        if m:
            ...
            if f == "性别":
                ...
                self.db.update_char(ch.gid, ch.uid, backstory=val[:1200])
        ...
        async with self._glock(ch.gid):
            ...
            if d.get("tags"):
                ...
                changed.append("性格")
            if d.get("backstory"):
                self.db.update_char(ch.gid, ch.uid, backstory=d["backstory"][:1200])
                changed.append("背景设定")
        if changed:
